...
config_userapiconfig(){
    #先删除缓存
    rm -rf /home/shadowsocks/*.pyc
    #写出配置
    cat > /home/shadowsocks/userapiconfig.py<<-EOF
# Config
NODE_ID = ${webid}
...
config_supervisord(){
  ...
  yum -y install supervisor
    ...
    [ -z "$(grep 'program:ssr' /etc/supervisord.conf)" ] && cat >> /etc/supervisord.conf << EOF
[program:ssr]
command=${python_install_dir}/bin/python /home/shadowsocks/server.py
startretries=36
EOF
  ...
  #----------------
    ...
    [ -z "$(grep 'program:ssr' /etc/supervisor/supervisord.conf)" ] && cat >> /etc/supervisor/supervisord.conf << EOF
[program:ssr]
command=${python_install_dir}/bin/python /home/shadowsocks/server.py
startretries=36
EOF
